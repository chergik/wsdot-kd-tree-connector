...
LatLon.intersection = function(path1start, path1brngEnd, path2start, path2brngEnd) {
    ...
    // there are two (antipodal) candidate intersection point
    var i1 = c1.cross(c2);
    var i2 = c2.cross(c1);
    console.log('intersections', i1.toLatLonS(), i2.toLatLonS())

    // bearing has direction which determines choice of intersection point; endpoints don't, so
    // choose closer alternative

    var intersection;
    intersection = Vector3d(0,0,0);
    switch (path1def+'/'+path2def) {
        case 'bearing/bearing':
            var d1 = path1start.toVector().cross(i1).dot(c1);
            var d2 = path1start.toVector().cross(i2).dot(c2);
            console.log('intersection b/b', d1, d2);
            break;
        case 'bearing/endpoint':
            break;
        case 'endpoint/bearing':
            break;
        case 'endpoint/endpoint': // select intersection in same hemisphere as 1st start point
            intersection = path1start.toVector().dot(i1) > 0 ? i1 : i2;
            break;
    }
    // console.log('intersection', path1start.toVector().dot(i1), path1start.toVector().dot(i2))
